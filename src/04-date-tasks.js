...
function timeSpanToString(startDate, endDate) {
  ...
  const mls = date.getUTCMilliseconds();
  let mlsStr = '';
  if (mls < 10) {
    mlsStr = `00${mls}`;
  } else if (mls < 100) {
  ...
  return `${hhStr}:${mmStr}:${ssStr}.${mlsStr}`;
}
...
function angleBetweenClockHands(date) {
  ...
  const hDeg = 0.5 * (60 * h + date.getUTCMinutes());
  const mDeg = 6 * date.getUTCMinutes();
  let ret = (Math.max(hDeg, mDeg) - Math.min(hDeg, mDeg));
  if (ret > 180) {
    ret -= 180;
  }
